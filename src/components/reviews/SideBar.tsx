...
export default function SidebarFilters({ title = "Categorías", items, onSelect }: Props) {
  return (
      ...
      <ul className="divide-y divide-border">
        {items.map((it) => (
          <li key={it.label}>
            ...
            >
              <span className={["text-sm", it.active ? "text-brown font-medium" : "text-foreground"].join(" ")}>
                {it.label}
              </span>

...
// // Árbol: provincia -> ciudad -> { barrio: count }
// export type Tree3 = Record<string, Record<string, Record<string, number>>>;

// export function useTriLevelFilter<T>(
//   data: T[],
//   opts: {
...
//   // Árbol + orden estable (evita hydration mismatch)
...
//   const [selL3, setSelL3] = useState<Record<string, Record<string, Set<string>>>>({}); // por prov→ciudad
...
//     setSelL3(prev => {
//       const byCity = prev[l1] ?? {};
//       const set = new Set(byCity[l2] ?? []);
//       set.has(l3) ? set.delete(l3) : set.add(l3);
//       return { ...prev, [l1]: { ...byCity, [l2]: set } };
//     });
//   }, []);
